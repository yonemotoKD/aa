...
bool Application::Init(int w, int h)
{
	...
	// ウィンドウ作成
	//===================================================================
	if (m_window.Create(w, h, "スライム探索物語", "Window") == false) {
...
void Application::Execute()
{
	...
	while (1)
	{
		...
		Math::Matrix ShatterMat = Math::Matrix::CreateTranslation(0.0f, 0.0f, -1.0f);
		
		//D3D.WorkShaderManager().m_cb8_Light.Work().DirLight_Color.x += 0.05f;
		...
		//空表示
		//D3D.WorkShaderManager().m_HD2DShader.DrawModel(sky,Math::Matrix::Identity, kBlackColor);
		D3D.WorkShaderManager().m_HD2DShader.DrawModel(sky);
