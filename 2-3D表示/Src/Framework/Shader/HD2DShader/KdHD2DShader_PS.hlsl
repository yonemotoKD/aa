...
Texture2D g_normalTex : register(t1);		// 法線マップ
Texture2D g_metalRoughTex : register(t2);	// メタリック/ラフネステクスチャ
Texture2D g_emissiveTex : register(t3);		// 発光テクスチャ
...
float4 main(VSOutput In) : SV_Target0
{
	...
	// Specular(反射色)
	{
		...
		color += ( g_DL_Color * spec ) * baseSpecular * baseColor.a * 0.5 * shadow;
	}
	...
	//	float3 envSpecular = g_environmentTex.SampleLevel(g_ss, vRef.xy, roughness * 9).rgb;
	//	envSpecular = pow(envSpecular, 1.0 / 2.2); // 元の画像がとある理由で暗くなってるので、元に戻す計算。

	//	// 光として加算する
	//	color += envSpecular * baseSpecular;
	//}
	...
	for( int i = 0; i < g_PointLightNum.x; i++ )
	{
		...
		if( dist < g_PointLights[ i ].Radius )
		{
			...
			//totalBrightness += atte * g_PointLights[ i ].IsBright;
			...
			// Diffuse(拡散光)
			{
				...
				// 光の色 * 材質の拡散色 * 透明率
				color += ( g_PointLights[ i ].Color * lightDiffuse ) * baseDiffuse * baseColor.a;
			}
			...
			// Specular(反射色)
			{
				...
				// 光の色 * 反射光の強さ * 材質の反射色 * 透明率 * 適当な調整値
				color += ( g_PointLights[ i ].Color * spec ) * baseSpecular * baseColor.a * 0.5;
			}
		}
	}
	...
	//{
	//	// 閾値とマスク値の差分で、縁を検出
	//	const float f = 0.03f; // 大きくすると輪郭が太くなる
	//	if (abs(discardValue - g_dissolveValue) < f)
	//	{
	//		color = float3(20, 5, 0.5);
	//	}
	//}
