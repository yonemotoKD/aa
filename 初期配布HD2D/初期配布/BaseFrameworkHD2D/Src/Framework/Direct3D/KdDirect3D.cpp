...
void KdDirect3D::ClientToWorld(const POINT& screenPos, float porjZ, Math::Vector3& dst)
{
	...
	// 各行列の逆行列を算出
	Math::Matrix invView = m_pShadermanager->m_cb7_Camera.Get().mView.Invert();
	invView.Invert();
	Math::Matrix invPrj = m_pShadermanager->m_cb7_Camera.Get().mProj.Invert();
...
void KdDirect3D::WorldToClient(const Math::Vector3& srcWorld, POINT& dst)
{
	Math::Matrix mVP;
	mVP._11 = m_windowWidth * 0.5f;
	mVP._22 = -m_windowHeight * 0.5f;
	...
	Math::Matrix convertMat = m_pShadermanager->m_cb7_Camera.Get().mView * m_pShadermanager->m_cb7_Camera.Get().mProj * mVP;
...
const KdShaderManager& KdDirect3D::GetShaderManager()
{
	assert(m_pShadermanager && "KdDirect3D::シェーダーマネージャが存在していません");

	return *m_pShadermanager;

}

KdShaderManager& KdDirect3D::WorkShaderManager()
{
	assert(m_pShadermanager && "KdDirect3D::シェーダーマネージャが存在していません");
...
bool KdDirect3D::Init(HWND hWnd, int w, int h, bool deviceDebug, std::string& errMsg)
{
	...
	*/
	...
	// シェーダー初期化
	m_pShadermanager = std::make_unique<KdShaderManager>();
