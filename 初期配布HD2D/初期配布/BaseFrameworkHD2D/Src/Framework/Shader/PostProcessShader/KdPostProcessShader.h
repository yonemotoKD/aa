...
public:
	...
	void BeginDrawPostProcess();
	void EndPostProcess();

	void BeginDrawBrightEffect();
	void EndDrawBrightEffect();

	void BlurProcess();
	void LightBloomProcess();
	void DepthOfFieldProcess();

	std::shared_ptr<KdTexture> WorkPostEffectTexture() { return m_postEffectRTPack.m_RTTexture; }
	std::shared_ptr<KdTexture> WorkPostDepthTexture() { return m_postEffectRTPack.m_ZBuffer; }
	std::shared_ptr<KdTexture> WorkBlurTexture() { return m_blurRTPack.m_RTTexture; }
	std::shared_ptr<KdTexture> WorkStrongBlurTexture() { return m_strongBlurRTPack.m_RTTexture; }
	std::shared_ptr<KdTexture> WorkDepthOfFieldTexture() { return m_depthOfFieldRTPack.m_RTTexture; }
...
private:
	...
	void GenerateBlurTexture(std::shared_ptr<KdTexture>& spSrcTex, std::shared_ptr<KdTexture>& spDstTex, D3D11_VIEWPORT& VP, int blurInfo);

	void DrawTexture(std::shared_ptr<KdTexture>* spSrcTex, int srcTexSize, std::shared_ptr<KdTexture>& spDstTex, D3D11_VIEWPORT& VP);
	...
	RenderTargetPack	m_postEffectRTPack;

	RenderTargetPack	m_blurRTPack;
	RenderTargetPack	m_strongBlurRTPack;

	RenderTargetPack	m_depthOfFieldRTPack;

	RenderTargetPack	m_brightEffectRTPack;
	static const int	kLightBloomNum = 4;
	RenderTargetPack	m_lightBloomRTPack[kLightBloomNum];

	RenderTargetChanger m_postEffectRTChanger;
	RenderTargetChanger m_brightRTChanger;
